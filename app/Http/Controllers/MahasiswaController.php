...
class MahasiswaController extends Controller
{
    ...
    public function firstTimeLogin(Request $request){
        ...
        $mahasiswa = session('first_login_data');

        if (!$mahasiswa || empty($mahasiswa->nrp)) {
            return redirect()->route('login');
        }

        return view('auth.profile-update', compact('mahasiswa'));
    }

    public function storeFirstTimeLogin(Request $request, ){

        $mahasiswa = session('first_login_data');
        $mahasiswa = Mahasiswa::where('nrp', $mahasiswa->nrp)->first();
        ...
        if (!$mahasiswa) {
            return redirect()->route('login')->withErrors([
                'username' => 'Data mahasiswa tidak ditemukan',
            ]);
        }
        ...
        session()->forget(['first_login', 'first_login_data']);
