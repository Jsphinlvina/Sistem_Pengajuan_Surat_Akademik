...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
    <div class="px-5 py-4 sm:px-6 sm:py-5">
        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
            {{ $mode === 'create' ? 'Tambah Periode Semester' : ($mode === 'edit' ? 'Edit Periode Semester' : 'Detail Periode Semester') }}
        </h3>
    </div>
    ...
    <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800 px-10">
        <form action="{{
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    <x-form-input name="nama" label="Nama Periode Semester" :value="$periodeSemester?->nama" :readonly="$isShow"
                        placeholder="Reguler Ganjil 2025/2026" />
                </div>
                <div>
                    @if($isEdit || $isShow)
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    <x-form-input name="kaprodi" label="Nama Kepala Program Studi" :value="$periodeSemester?->kaprodi" :readonly="$isShow"
            ...
            <div class="mt-10">
                ...
                <x-comfirm-delete :action="route('periode-semester.destroy', $periodeSemester->id)" message="Yakin ingin menghapus Periode Semester ini?">
                  <x-slot:trigger>
        ...
        <div class="rounded-2xl border bg-yellow-50 p-6 mt-8">
            <form action="{{ route('periode-semester.mahasiswa-periode-semester.store-import', $periodeSemester) }}" method="POST">
                @csrf
                ...
                <x-table>
                    ...
                    <x-slot:head>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            @foreach(['No','NRP', 'Status', 'Deskripsi'] as $column)
            ...
            <tr class="odd:bg-gray-100">
                ...
                <td class="px-5 py-4 sm:px-6">
                    <div class="flex items-center">
                       ...
                       <x-comfirm-delete :action="route('mahasiswa-periode-semester.destroy', $mahasiswaPeriodeSemester->id)" message="Yakin ingin menghapus Data Status Mahasiswa ini?">
                          <x-slot:trigger>
                             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
