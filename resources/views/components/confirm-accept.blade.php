...
'action',
'title' => 'Confirm Data',
'message' => 'Apakah Anda yakin ingin mensetujui data ini?',
'buttonText' => 'Setujui',
'method' => 'POST'
])

<div x-data="{isModalOpen: false}" class="inline">
  <span @click="isModalOpen = true" class="cursor-pointer">
    {{ $trigger }}
  </span>
  ...
  <div x-show="isModalOpen" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto modal z-99999">
    ...
    <div @click.outside="isModalOpen = false"
      ...
      <button @click="isModalOpen = false"
        class="absolute right-3 top-3 flex h-9.5 w-9.5 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:hover:text-white">
        ✕
      </button>
      <div class="text-center">
        <div class="relative flex items-center justify-center z-1 mb-7">
          <svg
            class="fill-success-50 dark:fill-success-500/15"
            width="90"
            height="90"
            viewBox="0 0 90 90"
            fill="none"
          ...
          <span
            class="absolute -translate-x-1/4 -translate-y-1/4 left-1/2 top-1/2"
          >
            <svg
        ...
        <div class="flex items-center justify-center w-full gap-3 mt-7">
          <button type="button" @click="isModalOpen=false"
            class="px-4 py-3 text-sm font-medium rounded-lg bg-gray-200 hover:bg-gray-300">
            Batal
          </button>
          <form action="{{ $action }}" method="POST">
            ...
            @method($method)
            <button type="submit"
              class="px-4 py-3 text-sm font-medium text-white rounded-lg bg-lime-500 hover:bg-lime-600">
              {{ $buttonText }}
            </button>
          </form>
        </div>
